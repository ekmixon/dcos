...
def get_zookeeper_mode():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while buf != b'':
            response += buf
            buf = sock.recv(512)
    if match := _zk_mode_pat.search(response):
        mode = match.group(1)
        result = zk_mode_map.get(mode)
        if result is None:
            raise KeyError(f'Unexpected mode: {mode} in {response}')
        return result
...
def wait(master_count_filename):
    ...
    log.info(f'Expected cluster size: {cluster_size}')
    ...
    try:
        ...
        log.error(f'Could not connect to exhibitor: {ex}')
        return
    if response.status_code != 200:
        log.error(
            f'Could not get exhibitor status: {EXHIBITOR_STATUS_URL}, Status code: {response.status_code}'
        )
