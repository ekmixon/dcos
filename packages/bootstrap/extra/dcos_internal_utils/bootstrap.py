...
class Bootstrapper(object):
    ...
    def cluster_id(self, path=utils.dcos_lib_path / 'cluster-id', readonly=False):
        zkid = None if readonly else str(uuid.uuid4()).encode('ascii')
        ...
        if utils.write_file_on_mismatched_content((zkid + '\n').encode('ascii'), path, utils.write_public_file):
            log.info(f'Wrote cluster ID to {path}')
        else:
            log.info(f'Cluster ID in ZooKeeper and file are the same: {zkid}')

        return zkid

    def _consensus(self, path, value, acl=None):
        if value is not None:
            log.info(f'Reaching consensus about znode {path}')
            try:
                self.zk.create(path, value, acl=acl)
                log.info(f'Consensus znode {path} created')
            except kazoo.exceptions.NodeExistsError:
                log.info(f'Consensus znode {path} already exists')
        self.zk.sync(path)
