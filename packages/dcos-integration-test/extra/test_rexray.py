...
def test_move_external_volume_to_new_agent(dcos_api_session: DcosApiSession) -> None:
    ...
    if (
        ...
        and expanded_config.get('platform') != 'aws'
    ):
    ...
    test_label = f'integration-test-move-external-volume-{test_uuid}'
    ...
    try:
        ...
        logging.info(f'Deleting volume: {test_label}')
        delete_cmd = f"/opt/mesosphere/bin/dcos-shell python /opt/mesosphere/active/dcos-integration-test/util/delete_ec2_volume.py {test_label}"

        delete_job = {
            'id': f'delete-volume-{test_uuid}',
            'run': {'cpus': 0.1, 'mem': 128, 'disk': 0, 'cmd': delete_cmd},
        }
        ...
        try:
            ...
            raise Exception(f'Failed to clean up volume {test_label}: {ex}') from ex
